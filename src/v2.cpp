...
// ---------------------- dispense one card ----------------------
const double DEG_PER_CARD = 320.0; // tune for your feeder
void dispenseOneCard(){
  const int MAX_MS = 360;   // watchdog
  double start = MotorDispense.position(deg);

  MotorDispense.setVelocity(90, percent);
  MotorDispense.spin(forward);

  timer t;
  while ((MotorDispense.position(deg) - start) < DEG_PER_CARD && t.time(msec) < MAX_MS) { }
  MotorDispense.stop(brake);
}

// ---------------------- main: deal to 4 players forever ----------------------
int main() {
  vexcodeInit();
  configureAllSensors();
